...
def plot_distribution_histograms(data):
    dim = data.shape[1]
    assert dim == 32 
    fig, axes = plt.subplots(nrows = 8, ncols = 4, figsize = (10,10), sharex=True)

    for i in range(len(axes)):
        for j in range(len(axes[0])):
            ax = axes[i][j]
            data_dim = data[:,i*4+j]
            ax.hist(data_dim,bins=50, color = 'green')
import seaborn as sns
import matplotlib.pyplot as plt

### Confusion Matrix

def confusion_matrix_clusters(y, y_pred):
